...
Pulls a tiny static ``jp-runtime.json.gz`` from this repo's GitHub Release on
first tool call so a dead endpoint can be repointed (edit runtime/jp-runtime.json
...
Opt-out: set ``JP_ELI_RUNTIME_URL=""`` to disable the fetch.
"""
...
MANIFEST_URL = os.environ.get(
    "JP_ELI_RUNTIME_URL",
    "https://github.com/matematicsolutions/jp-eli-mcp/releases/latest/download/jp-runtime.json.gz",
)
_CACHE_NAME = "jp-runtime.json"
_TIMEOUT = 15
_USER_AGENT = "jp-eli-mcp-runtime (+https://github.com/matematicsolutions/jp-eli-mcp)"

_runtime: dict[str, Any] | None = None


def _cache_dir() -> Path:
    env = os.environ.get("JP_ELI_CACHE_DIR")
    if env:
        return Path(env).expanduser()
    return Path.home() / ".matematic" / "cache" / "jp-eli"
